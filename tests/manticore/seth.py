...
class ManticoreEVM(Manticore):
    ...
    def create_contract_account(self, owner_account, source_code=None, contract_json=None):
        assert contract_json is not None

        ## Load compiled contract data
        name,src,bytecode,meta,meta_run,hashes = self.load_combined_json(source_code, contract_json)
        ## Create contract address
        addr = self.create_contract(owner_account, init=bytecode)
        ## Set internal contract metadata
        self.context['seth']['metadata'][addr] = name,src,bytecode,meta,meta_run,hashes 

        ## Return contract account
        return EVMContract(addr, self, default_caller=owner_account)


    def load_combined_json(self, source_code, combined_json):
        outp = json.loads(combined_json)
        assert len(outp['contracts']), "Only one contract by file supported"
        name, outp = outp['contracts'].items()[0]
        name = name.split(':')[1]
        bytecode = outp['bin'].decode('hex')
